...
__global__
void find_hits (const uint32_t* __restrict__  d_index_table, const uint32_t* __restrict__ d_pos_table, uint64_t*  d_seed_offsets, uint32_t seed_size, uint32_t* seed_hit_num, int num_hits, Anchor* d_hit, uint32_t start_seed_index, uint32_t start_hit_index){
    ...
}
...
int InitializeProcessor (int num_gpu, bool transition, uint32_t WGA_CHUNK, uint32_t input_seed_size, int* sub_mat, int input_xdrop, int input_hspthresh, bool input_noentropy){
    ...
    for(int g = 0; g < NUM_DEVICES; g++){
        ...
        g_cuda_ungapped_extenders.emplace_back(g, sub_mat, input_xdrop, input_noentropy, stream);
...
std::vector<segment> SeedAndFilter (std::vector<uint64_t> seed_offset_vector, bool rev, uint32_t buffer, int input_hspthresh){
    ...
    if(num_hits > 0){
        ...
        for(int i = 0; i < num_iter; i++){
            ...
            if(rev){
                if(!g_cuda_ungapped_extenders[g].ungapped_extend(d_query_rc_seq[buffer*NUM_DEVICES+g], query_length[buffer], d_ref_seq[g], ref_len, input_hspthresh, d_hit[g], iter_num_hits, d_hsp[g], d_num_anchors));
                {
                    //err...
                }
            }
            else{
                if(!g_cuda_ungapped_extenders[g].ungapped_extend(d_query_seq[buffer*NUM_DEVICES+g], query_length[buffer], d_ref_seq[g], ref_len, input_hspthresh, d_hit[g], iter_num_hits, d_hsp[g], d_num_anchors));
                {
                    //err...
                }
            }
            GW_CU_CHECK_ERR(cudaMemcpyAsync(&num_anchors[i], d_num_anchors, sizeof(int32_t), cudaMemcpyDeviceToHost, g_cuda_streams[g]));
