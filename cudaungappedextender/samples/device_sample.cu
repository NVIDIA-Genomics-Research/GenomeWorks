...
int main(int argc, char *argv[]) {
    ...
    // CSV Anchors file - Each row -> query_position_in_read_,
    // target_position_in_read_
    std::string anchors_file_path = "../data/example_hits.csv";

    std::vector<Anchor> h_hits;
    // Following function loops through all hits in the anchors csv and returns
    // results in
    // the passed vector
    ...
    GW_CU_CHECK_ERR(cudaMemcpyAsync(d_target, h_encoded_target.c_str(), sizeof(char) * h_encoded_target.size(),
                                    cudaMemcpyHostToDevice, stream0.get()));
    GW_CU_CHECK_ERR(cudaMemcpyAsync(d_hits, &h_hits[0], sizeof(Anchor) * h_hits.size(), cudaMemcpyHostToDevice,
                                    stream0.get())));
    ...
    if (h_num_hsps > 0) {
        ...
        for (const auto &segment : h_hsps) {
            ...
            std::cout << "Position in query: "
                      << segment.anchor.query_position_in_read_ << std::endl;
            std::cout << "Position in target: "
                      << segment.anchor.target_position_in_read_ << std::endl;
            i++;
        }
