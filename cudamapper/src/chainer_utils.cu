                            ...
                            cudaStream_t& _cuda_stream)
{
    ...
    device_buffer<int32_t> d_chain_starts(num_overlaps, _allocator, _cuda_stream);  // Index of the first anchor in the anchors array for an overlap.
    device_buffer<int32_t> d_chain_lengths(num_overlaps, _allocator, _cuda_stream); // Equivalent to num_residues

    cub::DeviceScan::ExclusiveSum(d_temp_storage,
                                  temp_storage_bytes,
                                  d_chain_lengths.data(),
                                  d_chain_starts.data(),
                                  num_overlaps,
                                  _cuda_stream);
    ...
    int32_t last_start  = cudautils::get_value_from_device(d_num_total_anchors.data() + num_overlaps - 1, _cuda_stream);
    int32_t last_length = cudautils::get_value_from_device(d_num_total_anchors.data() + num_overlaps - 1, _cuda_stream);
    num_total_anchors   = last_start + last_length;
