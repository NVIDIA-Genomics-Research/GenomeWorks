                 ...
                 std::vector<std::vector<Index::RepresentationToSketchElements>>& read_id_and_representation_to_sketch_elements)
{
    ...
    auto build_index_lambda = [number_of_reads,
                               ...
                               &sections_for_threads](std::uint32_t section_id) {
        ...
        for (std::size_t sketch_element_index = first_element_in_this_section + 1; sketch_element_index < last_element_in_this_section; ++sketch_element_index)
        {
            ...
            if (input_representations[sketch_element_index] != current_representation)
            { // new representation -> save data for the previous one
            ...
            { // still the same representation
                if (input_readids_positions_directions[sketch_element_index].read_id_ != current_read_id)
                { // new read_id -> save the data for the previous one
...
template <typename SketchElementImpl>
const std::vector<read_id_t>& IndexGPU<SketchElementImpl>::read_ids() const
{
    return read_ids_;
}

template <typename SketchElementImpl>
const std::vector<typename SketchElementImpl::DirectionOfRepresentation>& IndexGPU<SketchElementImpl>::directions_of_reads() const
{
...
template <typename SketchElementImpl>
bool IndexGPU<SketchElementImpl>::reached_end_of_input() const
{
    return reached_end_of_input_;
}
...
template <typename SketchElementImpl>
const std::vector<std::vector<Index::RepresentationToSketchElements>>& IndexGPU<SketchElementImpl>::read_id_and_representation_to_sketch_elements() const
{
    return read_id_and_representation_to_sketch_elements_;
}
...
void IndexGPU<SketchElementImpl>::generate_index(const std::vector<io::FastaParser*>& parsers, const std::vector<std::pair<std::uint64_t, std::uint64_t>>& read_ranges)
{
    ...
    read_id_t read_id = 0;
    for (auto fasta_object_id : fasta_sequence_indices)
    { //TODO do not start from zero
