...
public:
    BasicAlignment(std::vector<uint8_t> nodes, SizeTVec2D outgoing_edges,
                   ...
                   std::vector<uint8_t> read, std::vector<int8_t> base_weights, std::vector<SizeT> alignment_graph, std::vector<SizeTTT> alignment_read)
    ...
    void get_alignments(SizeT* alignment_graph, SizeTTT* alignment_read, uint16_t* alignment_length) const
    {
    ...
    void get_alignment_buffers(SizeT* alignment_graph, SizeTTT* alignment_read, uint16_t* alignment_length,
...
protected:
    ...
    std::vector<SizeTTT> alignment_read_;
};
...
BasicGraph testAddAlignment(const BasicAlignment& obj)
{
    ...
    CGA_CU_CHECK_ERR(cudaMallocManaged((void**)&alignment_read, batch_size.max_sequence_size * sizeof(SizeTTT)));
    CGA_CU_CHECK_ERR(cudaMallocManaged((void**)&node_coverage_counts, batch_size.max_nodes_per_window * sizeof(SizeTTT)));
    CGA_CU_CHECK_ERR(cudaMallocManaged((void**)&sequence_begin_nodes_ids, batch_size.max_sequences_per_poa * sizeof(SizeT)));
    CGA_CU_CHECK_ERR(cudaMallocManaged((void**)&outgoing_edges_coverage, batch_size.max_nodes_per_window * CUDAPOA_MAX_NODE_EDGES * batch_size.max_sequences_per_poa * sizeof(uint16_t)));
    CGA_CU_CHECK_ERR(cudaMallocManaged((void**)&outgoing_edges_coverage_count, batch_size.max_nodes_per_window * CUDAPOA_MAX_NODE_EDGES * sizeof(uint16_t)));
