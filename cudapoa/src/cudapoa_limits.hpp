...
static bool use32bitScore(const BatchConfig& batch_size, bool banded, const int16_t gap_score, const int16_t mismatch_score, const int16_t match_score)
{
    ...
    int32_t max_num_nodes = banded ? batch_size.max_nodes_per_graph_banded : batch_size.max_nodes_per_graph;
...
static bool use32bitSize(const BatchConfig& batch_size, bool banded)
{
    int32_t max_length = batch_size.max_consensus_size;
    if (banded)
    {
        max_length = std::max(max_length, batch_size.max_nodes_per_graph_banded);
    }
    else
    {
