...
static bool use32bitScore(const BatchSize& batch_size, bool banded, const int16_t gap_score, const int16_t mismatch_score, const int16_t match_score)
{
    ...
    int32_t max_num_nodes = banded ? batch_size.max_nodes_per_window_banded : batch_size.max_nodes_per_window;
...
static bool use32bitSize(const BatchSize& batch_size, bool banded)
{
    ...
    if (banded)
    {
        max_length = std::max(max_length, batch_size.max_nodes_per_window_banded);
    }
    else
    {
        max_length = std::max(max_length, batch_size.max_nodes_per_window);
    }
