...
void BatchBlock::calculate_size()
{
    ...
    total_h_ += output_size_ * sizeof(uint16_t); // output_details_h_->coverage
    total_h_ += sizeof(OutputDetails);           // output_details_d_
    // for output - device
    total_d_ += output_size_ * sizeof(uint8_t);  // output_details_d_->consensus
    total_d_ += output_size_ * sizeof(uint16_t); // output_details_d_->coverage
    ...
    total_d_ += sizeof(int32_t) * max_nodes_per_window_ * max_poas_;                                // graph_details_d_->consensus_scores
    total_d_ += sizeof(int16_t) * max_nodes_per_window_ * max_poas_;                                // graph_details_d_->consensus_predecessors
