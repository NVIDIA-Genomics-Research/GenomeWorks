...
{
public:
    BatchBlock(int32_t device_id_, size_t avail_mem, int32_t max_sequences_per_poa, int8_t output_mask, const UpperLimits& max_limits, bool banded_alignment = false);
    ~BatchBlock();
...
protected:
    ...
    std::tuple<int64_t, int64_t, int64_t, int64_t> calculate_space_per_poa(const UpperLimits& max_limits);
