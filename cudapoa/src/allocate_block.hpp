...
{
public:
    BatchBlock(uint32_t device_id_, uint32_t max_poas, uint32_t max_sequences_per_poa, bool banded_alignment = false);
    ~BatchBlock();
...
protected:
    ...
    uint32_t matrix_sequence_dimension_;
    uint32_t max_graph_dimension_;
