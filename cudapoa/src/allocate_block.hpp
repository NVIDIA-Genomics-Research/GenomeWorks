...
public:
    ...
    void get_graph_details(GraphDetails** graph_details_d_p, GraphDetails** graph_details_h_p)
    {
        ...
        offset_d_ += cudautils::align<int64_t, 8>(sizeof(uint16_t) * max_nodes_per_window_ * max_poas_);
        graph_details_d->outgoing_edges = reinterpret_cast<SizeTT*>(&block_data_d_[offset_d_]);
        ...
        graph_details_d->sorted_poa_node_map = reinterpret_cast<SizeTTT*>(&block_data_d_[offset_d_]);
...
protected:
    ...
    std::tuple<int64_t, int64_t, int64_t, int64_t> calculate_space_per_poa(const BatchSize& batch_size)
    {
        ...
        device_size_per_poa += sizeof(SizeTT) * max_nodes_per_window_ * CUDAPOA_MAX_NODE_EDGES * poa_count;                                                                   // graph_details_d_->outgoing_edges
        ...
        device_size_per_poa += sizeof(SizeTTT) * max_nodes_per_window_ * poa_count;                                                                                          // graph_details_d_->sorted_poa_node_map
