                             ...
                             int32_t rerun)
{
    ...
    for (int32_t graph_pos = 0; graph_pos < graph_count; graph_pos++)
    {
        ...
        for (int32_t read_pos = lane_idx * CUDAPOA_CELLS_PER_THREAD + band_start; read_pos < band_start + band_width; read_pos += WARP_SIZE * CUDAPOA_CELLS_PER_THREAD)
        {
            ...
            int64_t score_index          = static_cast<int64_t>((read_pos - band_start) / CUDAPOA_CELLS_PER_THREAD + static_cast<float>(score_gIdx) * static_cast<float>(band_width / CUDAPOA_CELLS_PER_THREAD + 2));
                                               ...
                                               bool adaptive)
{
    ...
    if (adaptive)
    {
        *aligned_nodes = runNeedlemanWunschBanded<uint8_t, int16_t, int16_t, true>(nodes,
                                                                                   graph,
                                                                                   node_id_to_pos,
                                                                                   graph_count,
                                                                                   incoming_edge_count,
    ...
    {
        *aligned_nodes = runNeedlemanWunschBanded<uint8_t, int16_t, int16_t, false>(nodes,
                                                                                    graph,
                                                                                    node_id_to_pos,
                                                                                    graph_count,
                                                                                    incoming_edge_count,
