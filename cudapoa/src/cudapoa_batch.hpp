...
{
public:
    CudapoaBatch(int32_t device_id, cudaStream_t stream, size_t max_mem, int8_t output_mask, BatchSize batch_size, int16_t gap_score = -8, int16_t mismatch_score = -6, int16_t match_score = 8, bool cuda_banded_alignment = false);
    ~CudapoaBatch();
