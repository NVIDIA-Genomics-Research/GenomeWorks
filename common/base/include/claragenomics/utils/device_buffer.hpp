...
public:
    ...
    /// \param streams CUDA streams to be associated with this buffer. Freeing the memory (either trough destructor, clear_and_free() or clear_and_resize()) calls cudaStreamSynchronize() on all those stream. If no stream is specified default stream (0) is used.
