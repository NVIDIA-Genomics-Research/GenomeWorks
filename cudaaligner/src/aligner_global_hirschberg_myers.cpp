...
{
    static constexpr int32_t n_cols = 64; // has to be at least 2 (hirschberg fwd + hirschberg bwd).
    Workspace(int32_t max_alignments, int32_t max_n_words, int32_t max_target_length, int32_t warps_per_alignment, cudaStream_t stream)
        : stackbuffer(max_alignments * hirschberg_myers_stackbuffer_size), pvs(max_alignments * warps_per_alignment, max_n_words * n_cols, stream), mvs(max_alignments * warps_per_alignment, max_n_words * n_cols, stream), scores(max_alignments * warps_per_alignment, std::max(max_n_words * n_cols, (max_target_length + 1) * 2), stream), query_patterns(max_alignments, max_n_words * 8, stream)
    {
    }
    ...
    : AlignerGlobal(max_query_length, max_target_length, max_alignments, stream, device_id)
{
    ...
    workspace_ = std::make_unique<Workspace>(max_alignments, ceiling_divide<int32_t>(max_query_length, sizeof(hirschbergmyers::WordType)), max_target_length, hirschberg_myers_warps_per_alignment, stream);
}
                                                 ...
                                                 int32_t num_alignments, cudaStream_t stream)
{
    ...
    hirschberg_myers_gpu(workspace_->stackbuffer, hirschberg_myers_stackbuffer_size, results_d, result_lengths_d, max_result_length,
                         ...
                         hirschberg_myers_warps_per_alignment,
                         stream);
}
