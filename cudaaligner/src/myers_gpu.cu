...
int32_t myers_compute_edit_distance(std::string const& target, std::string const& query)
{
    ...
    cudautils::device_copy_n_async<char>(query.data(), get_size(query), sequences_d.data(), stream.get());
    cudautils::device_copy_n_async<char>(target.data(), get_size(target), sequences_d.data() + max_sequence_length, stream.get());
    cudautils::device_copy_n_async<int32_t>(lengths.data(), 2, sequence_lengths_d.data(), stream.get());
